...
export type ConstitutionalContext = {
  action: string;
  jurisdiction: string;
  destructive?: boolean;
  payload?: unknown;
...
export function enforceConstitution(ctx: ConstitutionalContext) {
  ...
  if (coercionSignals.some((term) => payloadText.includes(term))) {
    throw new ActionableError(
      `Action '${ctx.action}' violates constitutional vow '${GOVERNANCE_BEADS.NEVER_COERCE}'. Remove coercive intent and resubmit with compliant purpose.`,
      { action: ctx.action, jurisdiction: ctx.jurisdiction }
    );
  }
