...
function AnimatedNumber({ value, duration = 800 }) {
  ...
  const prev = useRef(0);
  useEffect(() => {
    const start = Date.now(), from = prev.current;
    prev.current = value;
    const anim = () => {
...
export default function LiveDemo({ onMintClick }) {
  ...
  useEffect(() => {
    ...
    const fillNext = () => {
      ...
      safe(() => {
        if (!alive.current) return;
        const d = DIMS[captured];
        if (!d) return;
        setScores(prev => ({ ...prev, [d.key]: val }));
        idx = captured + 1;
        safe(fillNext, 300);
      }, 500);
    };
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      c += 2;
      setHashRevealed(Math.min(c, 64));
      if (c >= 64) {
        clearInterval(iv);
        const weights = { labor:0.2, exchange:0.2, equality:0.2, presence:0.15, ratification:0.15, continuity:0.1 };
        setScores(prev => {
          let total = 0;
          for (const k in weights) total += (prev[k] || 0) * weights[k];
          const score   = Math.round(total * 10);
          const degrees = Math.round((score / 100) * 360 * 100) / 100;
          setBlocks(pb => {
            const prevDeg = pb.reduce((s, b) => s + b.degrees, 0);
            return [...pb, { id: pb.length+1, score, degrees, totalDegrees: prevDeg+degrees, hash, prevHash: pb.length>0 ? pb[pb.length-1].hash : "genesis" }];
          });
          safe(() => setStep(4), 800);
          return prev;
        });
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        ...
        {blocks.length>0 && step===4 && (
          <button className="btn-ghost" style={{ fontSize:10, padding:"4px 12px" }}
            onClick={() => { timers.current.forEach(clearTimeout); setBlocks([]); setStep(0); setUsername(""); setScores({}); }}>
            restart
          </button>
        )}
      </div>
      ...
      {step===0 && (
        <div style={{ maxWidth:380, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ fontSize:13, color:"var(--t)", marginBottom:24, lineHeight:1.6 }}>Enter any username. Watch the protocol score, hash, and seal a block in real time.</div>
          <div style={{ display:"flex", gap:8 }}>
            <input className="input" placeholder="pick a username" value={username}
              onChange={e => setUsername(e.target.value)}
              onKeyDown={e => e.key==="Enter" && handleCreatePassport()}
              style={{ flex:1 }} />
            <button className="btn-prime" onClick={handleCreatePassport} disabled={!username.trim()} style={{ whiteSpace:"nowrap" }}>mint demo</button>
          </div>
        </div>
      )}
            ...
            {DIMS.map((d, i) => {
              ...
              return (
                <div key={d.key} style={{ display:"flex", alignItems:"center", gap:12, padding:"6px 0", borderBottom: i<5?"1px solid rgba(124,58,237,0.05)":"none" }}>
                  <div className="m" style={{ fontSize:11, width:100, color:active?d.color:"var(--t2)", fontWeight:active?600:400, transition:"color 0.3s" }}>{d.label}</div>
                  <div style={{ flex:1, height:6, background:"rgba(255,255,255,0.03)", borderRadius:3, overflow:"hidden" }}>
                    <div style={{ height:"100%", borderRadius:3, width:val?(val*10)+"%":filling?"60%":"0%", background:d.color, transition:val?"width 0.4s ease-out":"width 0.3s", opacity:val?1:filling?0.3:0 }} />
                  </div>
        ...
        <div style={{ maxWidth:520, margin:"0 auto" }}>
          <div style={{ display:"flex", gap:20, alignItems:"center", padding:20, background:"rgba(10,10,16,0.6)", borderRadius:12, border:"1px solid var(--b)", marginBottom:16 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ marginTop:4, marginBottom:12 }}><span className="tag" style={{ color:tier.color, background:tier.bg }}>{tier.name}</span></div>
              <div style={{ display:"flex", gap:20 }}>
                <div><div className="m" style={{ fontSize:20, fontWeight:700, color:"var(--a)" }}><AnimatedNumber value={latestScore} /></div><div className="m" style={{ fontSize:8, color:"var(--t2)", letterSpacing:"0.1em" }}>SCORE</div></div>
                <div><div className="m" style={{ fontSize:20, fontWeight:700, color:"var(--a2)" }}><AnimatedNumber value={parseFloat((totalDegrees/360).toFixed(2))} /></div><div className="m" style={{ fontSize:8, color:"var(--t2)", letterSpacing:"0.1em" }}>REVOLUTIONS</div></div>
                <div><div className="m" style={{ fontSize:20, fontWeight:700, color:"#10b981" }}>{blocks.length}</div><div className="m" style={{ fontSize:8, color:"var(--t2)", letterSpacing:"0.1em" }}>BLOCKS</div></div>
              </div>
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 16px", background:"rgba(16,185,129,0.03)", borderRadius:8, border:"1px solid rgba(16,185,129,0.1)", marginBottom:16 }}>
            ...
            <span className="m" style={{ fontSize:9, color:"var(--t2)" }}>{blocks[0]?.hash.slice(0,8)}...{latestBlock?.hash.slice(-8)}</span>
          </div>
          ...
          <div style={{ padding:14, background:"rgba(124,58,237,0.03)", borderRadius:8, border:"1px solid var(--b)", textAlign:"center", marginBottom:16 }}>
            ...
            <div className="m" style={{ fontSize:11 }}>
              <span style={{ color:"var(--t2)" }}>quantumpassai.vercel.app/verify/</span>
              <span style={{ color:"var(--a)" }}>{passportId.slice(0,8)}</span>
            </div>
            <div style={{ fontSize:11, color:"var(--t)", marginTop:8, lineHeight:1.5 }}>Anyone with this link sees your score, tier, degree count, and full chain integrity.</div>
          </div>
          <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
            <button className="btn-prime" onClick={() => { timers.current.forEach(clearTimeout); setStep(2); }} style={{ fontSize:11 }}>seal another block</button>
            <button className="btn-ghost" onClick={onMintClick} style={{ fontSize:11 }}>mint real passport</button>
          </div>
        </div>
      )}
    </div>
