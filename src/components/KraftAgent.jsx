...
async function executeAction(action, apiKey) {
  ...
  switch (action.type) {
    case "create_passport": {
      const cp = await fetch(API + "/passport", {
        method: "POST",
        headers,
    ...
    case "create_key": {
      const ck = await fetch(API + "/key/create", {
        method: "POST",
        headers,
        body: JSON.stringify({ passport_id: action.passport_id }),
      });
    ...
    case "submit_score": {
      if (!apiKey) return { label: "Error", data: { error: "No API key." } };
      const ss = await fetch(API + "/score", {
        method: "POST",
        headers,
        body: JSON.stringify({
          labor: action.labor || 0,
          exchange: action.exchange || 0,
          equality: action.equality || 0,
          presence: action.presence || 0,
          ratification: action.ratification || 0,
          continuity: action.continuity || 0,
        }),
      });
...
export default function KraftAgent({ currentPage }) {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: 'Kraft-01 online. I run QuantumPass. Say "create passport" to get started, or ask me anything.',
      result: null,
    },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Connection failed: " + e.message, result: null },
      ]);
    }
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ color: "#64748b", fontSize: 11 }}>
                {apiKey ? "Authenticated" : "QuantumPass Agent"}
              </span>
            </div>
          </div>
