...
export const FREEBUFF_DEFAULT_MODEL = "openai/gpt-oss-120b";
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error("API rejected payload: " + r.status + (errBody ? " - " + errBody.slice(0, 200) : ""));
      }

      const d = await r.json();
      const text = (d.choices && d.choices[0] && d.choices[0].message && d.choices[0].message.content) || "";
      if (!text) throw new Error("Returned empty content");
      return text;
    } catch (e) {
      lastErr = e;
